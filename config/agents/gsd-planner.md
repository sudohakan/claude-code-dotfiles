...
depends_on: []              # CRITICAL for DAG scheduling: list specific plan IDs this plan requires.
                            # Empty [] = no dependencies (can run immediately).
                            # Example: ["03-01", "03-02"] = waits for both plans to complete.
                            # DO NOT leave empty if this plan uses output from another plan.
...
**DAG Scheduling Note:** Wave numbers serve as human-readable hints. At execution time,
the `depends_on` field drives actual scheduling — each plan starts as soon as its
specific dependencies complete, NOT when the entire wave completes.
Therefore: always populate `depends_on` accurately. Wave numbers should be consistent
with depends_on but are secondary to the explicit dependency declarations.
</step>
