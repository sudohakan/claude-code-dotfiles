...
async function main() {
  ...
  switch (command) {
    ...
    case 'phase-plan-dag': {
      phase.cmdPhasePlanDag(cwd, args[1], raw);
      break;
    }

    case 'update-dag-status': {
      phase.cmdUpdateDagStatus(cwd, args[1], args[2], raw);
      break;
    }

    case 'analyze-plan-routing': {
      phase.cmdAnalyzePlanRouting(cwd, args[1], raw);
      break;
    }
