...
function cmdPhasePlanDag(cwd, phase, raw) {
  if (!phase) {
    error('phase required for phase-plan-dag');
  }
  ...
  if (!phaseDir) {
    output({ phase: normalized, error: 'Phase not found', ready: [], blocked: {}, graph: {}, has_cycle: false }, raw);
    return;
  }
  ...
  for (const planFile of planFiles) {
    ...
    if (completedPlanIds.has(planId)) continue; // skip completed
    ...
    if (fm.depends_on) {
      ...
      // Backward compat: infer from wave — depends on all plans in previous waves
      const wave = parseInt(fm.wave, 10) || 1;
      if (wave > 1) {
        for (const otherFile of planFiles) {
          const otherId = otherFile.replace('-PLAN.md', '').replace('PLAN.md', '');
          if (otherId === planId) continue;
          const otherContent = fs.readFileSync(path.join(phaseDir, otherFile), 'utf-8');
          const otherFm = extractFrontmatter(otherContent);
          const otherWave = parseInt(otherFm.wave, 10) || 1;
          if (otherWave < wave) dependsOn.push(otherId);
        }
      }
    }
    ...
    graph[planId] = dependsOn.filter(d => allPlanIds.has(d));
    planMeta[planId] = {
      files_modified: filesModified,
      task_count: taskMatches.length,
      autonomous: fm.autonomous !== undefined ? (fm.autonomous === 'true' || fm.autonomous === true) : true,
      objective: fm.objective || null,
    };
  }
  ...
  for (const [planId, deps] of Object.entries(graph)) {
    const waitingOn = deps.filter(d => !completedPlanIds.has(d));
    if (waitingOn.length === 0) {
      ready.push(planId);
    } else {
      blocked[planId] = { waiting_on: waitingOn, all_deps: deps };
    }
  }
  ...
  output({
    ...
    graph,
    plan_meta: planMeta,
    completed: [...completedPlanIds],
    has_cycle: hasCycle,
  }, raw);
}

function cmdUpdateDagStatus(cwd, phase, completedPlanId, raw) {
  if (!phase || !completedPlanId) {
    error('phase and completed_plan_id required for update-dag-status');
  }

  // Re-run DAG computation with the assumption that completedPlanId is now done
  // The SUMMARY.md file should already exist on disk at this point
  // So we just re-run cmdPhasePlanDag which checks SUMMARY.md existence
  cmdPhasePlanDag(cwd, phase, raw);
}

function cmdAnalyzePlanRouting(cwd, phase, raw) {
  if (!phase) {
    error('phase required for analyze-plan-routing');
  }
  ...
  if (!phaseDir) {
    output({ phase: normalized, error: 'Phase not found', routing: {} }, raw);
    return;
  }
  ...
  // Detect file overlaps between plans
  const fileOwners = {};
  ...
  for (const [file, owners] of Object.entries(fileOwners)) {
    if (owners.length > 1) {
      for (const owner of owners) {
        if (!overlapping[owner]) overlapping[owner] = new Set();
        owners.filter(o => o !== owner).forEach(o => overlapping[owner].add(o));
      }
    }
  }
  ...
  const routing = {};
  for (const plan of planData) {
    const hasOverlap = overlapping[plan.id] && overlapping[plan.id].size > 0;
    const isLarge = plan.files_modified.length >= 5 || plan.task_count >= 8;

    if (isLarge) {
      routing[plan.id] = {
        executor: 'cs-spawn',
        reason: `large plan (${plan.task_count} tasks, ${plan.files_modified.length} files)`,
      };
    } else if (hasOverlap) {
      routing[plan.id] = {
        executor: 'cs-spawn',
        reason: `file overlap with ${[...overlapping[plan.id]].join(', ')}`,
      };
    } else {
      routing[plan.id] = {
        executor: 'task',
        reason: `small plan (${plan.task_count} tasks, ${plan.files_modified.length} files)`,
      };
    }
  }

  output({ phase: normalized, routing }, raw);
}
